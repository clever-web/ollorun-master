...
const ComplianceServiceComponent = ({ isRefund, isChanged }) => {
    ...
    const emptyForm = () => {
        setFName(""); setLName(""); setUserName("");
        setEmail(""); setPhone(""); setOrdernumber("");
    ...
    const validateForm = () => {
        if (fName && lName && username && email &&
            ...
            (isRefund ? ordernumber : !ordernumber) && message && capchaResult
        )
            return true;
        else return false;
    }
        ...
        if (validateForm()) {
            ...
            try {
                const response = await axios({
                    method: "post",
                    url: `${API_URL}/contact/compliance`,
                    data: {
                        type: isRefund ? "refund" : "ethics",
                        fName: fName,
                        lName: lName,
                        username: username,
                        email: email,
...
export default ComplianceServiceComponent;